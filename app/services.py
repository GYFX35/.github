...
def parse_affiliate_csv(
    file_stream: io.StringIO,
) -> Tuple[List[AffiliatePerformanceData], List[str]]:
    # ... (content as before) ...
    ...
    if missing_required_headers:
        errors.append(
            f"Missing required CSV headers: {', '.join(missing_required_headers)}"
        )
        return data_records, errors
    unexpected_headers = [h for h in reader.fieldnames if h not in all_expected_headers]
    if unexpected_headers:
        current_app.logger.warning(
            f"CSV contains unexpected headers (will be ignored): {', '.join(unexpected_headers)}"
        )
        ...
        try:
            ...
            def safe_to_int(value_str: str, field_name: str) -> Optional[int]:
                ...
                try:
                    ...
                    errors.append(
                        f"Row {row_num}: Invalid integer value '{value_str}' for '{field_name}'."
                    )
                    return None
            ...
            def safe_to_float(value_str: str, field_name: str) -> Optional[float]:
                ...
                try:
                    ...
                    errors.append(
                        f"Row {row_num}: Invalid float value '{value_str}' for '{field_name}'."
                    )
                    return None
            ...
            data = AffiliatePerformanceData(
                ...
                commission_amount=safe_to_float(
                    row.get("commission_amount"), "commission_amount"
                ),
            )
            data_records.append(data)
        except ValueError as e:
            errors.append(
                f"Row {row_num}: Error parsing data. Invalid date format for '{report_date_str}'? Expected YYYY-MM-DD. Details: {e}. Skipping row."
            )
        except Exception as e:
            errors.append(
                f"Row {row_num}: An unexpected error occurred processing this row: {e}. Skipping row."
            )
    return data_records, errors


def parse_ad_campaign_csv(
    file_stream: io.StringIO,
) -> Tuple[List[AdCampaignPerformanceData], List[str]]:
    # ... (content as before) ...
    ...
    for row_num, row in enumerate(reader, start=2):
        try:
            ...
            platform = platform.strip() if platform is not None else None
...
def initialize_fb_api(access_token: str = None) -> bool:
    ...
    try:
        ...
        current_app.logger.info(
            "FacebookAdsApi initialized successfully for this request."
        )
        return True
    except Exception as e:
...
def get_fan_ad_placements_mock():
    if not _fb_api_initialized_this_request and not initialize_fb_api():
        raise Exception(
            "Facebook API not initialized. Please connect to Facebook first."
        )
...
def get_fan_performance_data_mock(
    placement_ids: List[str] = None, date_preset: str = "last_7d"
):
    if not _fb_api_initialized_this_request and not initialize_fb_api():
        raise Exception(
            "Facebook API not initialized. Please connect to Facebook first."
    ...
    current_app.logger.info(
        f"Fetching MOCK FAN performance data for placements: {placement_ids or 'all mock'} with date_preset: {date_preset}"
    )
    data_to_return = []
    target_placements = (
        get_fan_ad_placements_mock()
        if not placement_ids
        else [p for p in get_fan_ad_placements_mock() if p["id"] in placement_ids]
    )
...
def get_google_ads_client(refresh_token: str = None) -> Optional[GoogleAdsClient]:
    ...
    if not refresh_token:
        current_app.logger.warning(
            "No Google Ads refresh token provided or in session."
        )
        return None
...
def list_accessible_google_ads_customers(
    ...
    if not client:
        current_app.logger.error(
            "GoogleAdsClient not provided to list_accessible_google_ads_customers."
        )
        return []
    ...
    try:
        ...
        current_app.logger.info(
            f"Found {len(accessible_customers_response.resource_names)} accessible customer resource names."
        )
            ...
            try:
                ...
                for batch in stream:
                    for row in batch.results:
                        ...
                        current_app.logger.info(
                            f"Fetched details for customer: {customer.descriptive_name} ({customer.id})"
                        )
                ...
                current_app.logger.error(
                    f"Unexpected error fetching details for customer {customer_id}: {e}"
                )
        ...
        current_app.logger.error(
            f"Unexpected error listing accessible Google Ads customers: {e}"
        )
        raise
...
def get_ai_cloud_recommendations(
    service_data: List[CloudServiceData],
) -> List[Dict[str, str]]:
