...
from .services import (
    parse_affiliate_csv,
    parse_ad_campaign_csv,
    initialize_fb_api,
    get_fan_ad_placements_mock,
    get_fan_performance_data_mock,
    get_google_ads_client,
    list_accessible_google_ads_customers,  # Added Google Ads services
    get_mock_cloud_service_data,
    get_ai_cloud_recommendations,  # Added Cloud Optimization services
)
...
def affiliate_marketing():
    ...
    return render_template(
        "affiliate_marketing.html", affiliate_data=current_app.affiliate_data_store
    )
...
def ads_optimization():
    ...
    if "fb_access_token" in session:
        ...
        if not initialize_fb_api(session.get("fb_access_token")):  # Pass token
            fb_error = (
                "Failed to initialize Facebook API. Token might be invalid or expired."
            )
        ...
        if action == "fetch_fan_data":
            if fb_connected and not fb_error:
                try:
                    ...
                    if fan_placements:
                        ...
                        fan_performance_data = get_fan_performance_data_mock(
                            placement_ids=mock_placement_ids
                        )
            ...
            if google_ads_connected:
                try:
                    ...
                    if (
                        not gads_refresh_token
                    ):  # Should not happen if google_ads_connected is true
                        raise Exception(
                            "Google Ads refresh token not found in session."
                        )

                    gads_client = get_google_ads_client(gads_refresh_token)
                    if gads_client:
                        google_ads_customers = list_accessible_google_ads_customers(
                            gads_client
                        )
                        if google_ads_customers:
                            flash(
                                f"Successfully fetched {len(google_ads_customers)} Google Ads customer(s).",
                                "success",
                            )
                        else:
                    ...
                    Exception
                ) as e:  # Catches GoogleAdsException from service or others
                    current_app.logger.error(
                        f"Error fetching Google Ads customers: {e}"
                    )
                    google_ads_error = str(e)
                    flash(
                        f"Error fetching Google Ads customers: {google_ads_error}",
                        "danger",
                    )
            else:
        ...
        if file and file.filename.endswith(".csv"):
            try:
                ...
                if new_data:
                    ...
                    flash(
                        f"Successfully uploaded and processed {len(new_data)} affiliate records.",
                        "success",
                    )
                elif not errors:
                    flash("CSV processed, but no new valid data found.", "warning")
                return redirect(url_for("main.affiliate_marketing"))
            except Exception as e:
                current_app.logger.error(f"Err affiliate CSV: {e}")
                flash(f"Err: {e}", "danger")
                return redirect(request.url)
        else:
            flash("Invalid file type. CSV only.", "danger")
            return redirect(request.url)
        ...
        if file and file.filename.endswith(".csv"):
            try:
                ...
                if new_data:
                    ...
                    flash(
                        f"Successfully processed {len(new_data)} ad campaign records.",
                        "success",
                    )
                elif not errors:
                    flash("CSV processed, but no new valid data found.", "warning")
                ...
                current_app.logger.error(f"Err ad CSV: {e}")
                flash(f"Err: {e}", "danger")
                return redirect(request.url)
        else:
            flash("Invalid file type. CSV only.", "danger")
...
def connect_facebook_fan():
    try:
        ...
        if not app_id or "YOUR_FACEBOOK_APP_ID" in app_id:
            flash("Facebook App ID not configured.", "danger")
            return redirect(url_for("main.ads_optimization"))
        scopes = ["read_audience_network_insights", "ads_read"]
        oauth_url = FacebookAdsApi.get_oauth_url(
            app_id=app_id, redirect_uri=redirect_uri, scope=scopes
        )
        return redirect(oauth_url)
    except Exception as e:
        current_app.logger.error(f"FB OAuth URL err: {e}")
        flash(f"FB Conn Err: {e}", "danger")
        return redirect(url_for("main.ads_optimization"))
...
def fb_oauth_callback():
    ...
    if not auth_code:
        flash("Facebook auth failed.", "danger")
        return redirect(url_for("main.ads_optimization"))
    try:
        ...
        s_token_info = FacebookAdsApi.get_oauth_token_from_code(
            app_id=app_id,
            app_secret=app_secret,
            redirect_uri=redirect_uri,
            code=auth_code,
        )
        l_token_info = FacebookAdsApi.get_long_lived_token(
            app_id=app_id,
            app_secret=app_secret,
            short_lived_token=s_token_info["access_token"],
        )
        session["fb_access_token"] = l_token_info["access_token"]
        flash("Successfully connected to Facebook!", "success")
    except FacebookRequestError as e:
        current_app.logger.error(f"FB API err: {e.api_error_message()}")
        flash(f"FB API Err: {e.api_error_message()}", "danger")
    except KeyError:
        current_app.logger.error("FB KeyError: Missing token data.")
        flash("FB Token Error: Missing data.", "danger")
    except Exception as e:
        current_app.logger.error(f"FB OAuth Err: {e}")
        flash(f"FB Err: {e}", "danger")
    return redirect(url_for("main.ads_optimization"))
...
def connect_google_ads():
    try:
        ...
        if (
            ...
            or "YOUR_GOOGLE_ADS_CLIENT_ID" in client_id
        ):
            flash("Google Ads credentials not fully configured.", "danger")
            return redirect(url_for("main.ads_optimization"))
        client_config = {
            "web": {
                "client_id": client_id,
        ...
        current_app.logger.error(f"Google Ads OAuth URL error: {e}")
        flash(f"Google Ads Connection Error: {e}", "danger")
        return redirect(url_for("main.ads_optimization"))
...
def google_ads_oauth_callback():
    state_in_session = session.pop("google_oauth_state", None)
    state_from_google = request.args.get("state")
    if not state_in_session or state_in_session != state_from_google:
        flash("OAuth state mismatch. CSRF suspected.", "danger")
        current_app.logger.error("GAds OAuth state mismatch.")
        return redirect(url_for("main.ads_optimization"))
    auth_code = request.args.get("code")
    if not auth_code:
        flash("Google Ads auth failed.", "danger")
        return redirect(url_for("main.ads_optimization"))
    try:
        client_id = current_app.config.get("GOOGLE_ADS_CLIENT_ID")
        client_secret = current_app.config.get("GOOGLE_ADS_CLIENT_SECRET")
        ...
        if not credentials.refresh_token:
            flash(
                "No Google Ads refresh token. Ensure offline access. Re-auth or revoke app access in Google settings.",
                "warning",
            )
            current_app.logger.warning("GAds OAuth: Refresh token not obtained.")
            if credentials.token:
                session["google_ads_access_token"] = credentials.token
                session["google_ads_access_token_expiry"] = (
                    credentials.expiry.isoformat() if credentials.expiry else None
                )
                flash("Connected (Access Token only). Short-lived.", "warning")
            else:
                raise Exception("Failed to obtain any token from Google.")
        else:
            session["google_ads_refresh_token"] = credentials.refresh_token
            current_app.logger.info("GAds: Refresh token obtained.")
            session.pop("google_ads_access_token", None)
            session.pop("google_ads_access_token_expiry", None)
            flash("Successfully connected to Google Ads!", "success")
    except Exception as e:
        current_app.logger.error(f"Google Ads OAuth Callback Error: {e}")
        flash(f"Google Ads Connection Error: {e}", "danger")
    return redirect(url_for("main.ads_optimization"))
